        ...
        <div
            ...
            <div className="flex-1">
                <h2 className="mb-2 text-lg font-semibold text-gray-900 dark:text-white">
                    Les actions derrière notre architecture orientée événements:
                </h2>
                {everSubmit ? (
                    <ul className="space-y-2 text-gray-500 list-inside dark:text-gray-400">
                        <li className="flex items-center text-black dark:text-white">
                            <div className="font-bold p-1">Le commentaire envoyé:</div>
                            {comment}
                        </li>
                        <li className="flex items-center">
                            <GreenCheckMark/>
                            Le commentaire a été envoyé à l'application microservices !
                        </li>
                        <li className="flex items-center">
                            <GreenCheckMark/>
                            Celui-ci a été transformé en événement CloudEvent

                        </li>
                        <li className="flex items-center">
                            <GreenCheckMark/>
                            Le microservices a peut-être transmis l'événement au Broker
                        </li>
                        <li className="flex items-center">
                            ...
                            {responseSuccess === "error"
                                ? "L'événement CloudEvent a été généré par le Broker, mais une erreur a été retournée."
                                : "L'événement CloudEvent a été généré par le Broker, en attente d'une réponse."}
                        </li>
                        <li className="flex items-center">
                            ...
                            Accusé de réception reçu, le cycle est terminé !
                        </li>
                        <li className="flex items-center">
                            {responseSuccess === "success" ? (
                                <GreenCheckMark/>
                            ...
                            Attendez quelques secondes jusqu'à ce que le système ait fini de traiter le commentaire
                        </li>
                    </ul>
                ) : (
                    <h1>Essayez d'envoyer quelque chose !</h1>
                )}
            </div>
            <KuackImage/>
        </div>
...
const CommentForm = () => {
    ...
    const handleSubmit = (event) => {
        ...
        fetch("http://localhost:8080/api/comments/broker", {
            method: "POST",
            headers: {
    ...
    return (
        <div>
          ...
          <textarea
              ...
              rows="3"
              placeholder="Laissez un commentaire (en anglais)"
              value={comment}
